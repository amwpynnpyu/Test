...
bool  CTimerEngine::StartService()
{
	...
	if (m_bService == true)
	{
		CTraceService::TraceString(TEXT("亿网棋牌定时器引擎重复启动，启动操作忽略"), TraceLevel_Warning);
		return true;
	}
	...
	{
		CTraceService::TraceString(TEXT("亿网棋牌定时器引擎线程服务初始化失败"), TraceLevel_Exception);
		return false;
	}
	...
	{
		CTraceService::TraceString(TEXT("亿网棋牌定时器引擎线程服务启动失败"), TraceLevel_Exception);
		return false;
	}
