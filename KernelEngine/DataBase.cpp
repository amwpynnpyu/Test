...
CDataBase::CDataBase() : m_dwResumeConnectCount(30L), m_dwResumeConnectTime(30L)
{
	...
	if (m_DBCommand == NULL) throw TEXT("亿网棋牌数据库命令对象创建失败");
	if (m_DBRecordset == NULL) throw TEXT("亿网棋牌数据库记录集对象创建失败");
	if (m_DBConnection == NULL) throw TEXT("亿网棋牌数据库连接对象创建失败");
...
wSinkCount = 1;
	for(WORD i = 0; i < wSinkCount; i ++)
	{
		...
		{
			CTraceService::TraceString(TEXT("亿网棋牌数据库引擎外挂服务接口获取失败，挂接操作失败"), TraceLevel_Exception);
			return false;
		}
...
bool  CDataBaseEngine::StartService()
{
	...
	if (m_bService == true)
	{
		CTraceService::TraceString(TEXT("亿网棋牌数据库引擎重复启动，启动操作忽略"), TraceLevel_Warning);
		return true;
	}
	...
	{
		CTraceService::TraceString(TEXT("亿网棋牌数据库引擎外挂服务不存在"), TraceLevel_Exception);
		return false;
	}
	...
	{
		CTraceService::TraceString(TEXT("亿网棋牌数据库引擎与队列服务绑定失败"), TraceLevel_Exception);
		return false;
	}
	...
	for(WORD i = 0; i < m_pIDataBaseEngineSinks.size(); i ++)
	{
		...
		{
			CTraceService::TraceString(TEXT("亿网棋牌数据库引擎外挂服务启动失败"), TraceLevel_Exception);
			return false;
		}
	}
	//设置变量
