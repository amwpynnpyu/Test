...
bool  CTCPNetworkEngine::SetTCPNetworkEngineEvent(IUnknownEx * pIUnknownEx)
{
	...
	if (m_bService == true)
	{
		CTraceService::TraceString(TEXT("亿网棋牌网络引擎处于服务状态，绑定操作忽略"), TraceLevel_Exception);
		return false;
	}
	...
	{
		CTraceService::TraceString(TEXT("亿网棋牌网络引擎与触发服务绑定失败"), TraceLevel_Exception);
		return false;
	}
...
bool  CTCPNetworkEngine::SetServiceParameter(WORD wServicePort, WORD wMaxConnect, LPCTSTR  pszCompilation)
{
	...
	if (m_bService == true)
	{
		CTraceService::TraceString(TEXT("亿网棋牌网络引擎处于服务状态，端口绑定操作忽略"), TraceLevel_Exception);
		return false;
	}
...
bool  CTCPNetworkEngine::StartService()
{
	...
	if (m_bService == true)
	{
		CTraceService::TraceString(TEXT("亿网棋牌网络引擎重复启动，启动操作忽略"), TraceLevel_Warning);
		return true;
	}
	...
	{
		m_wListenPort = 3000;
		CTraceService::TraceString(TEXT("亿网棋牌网络引擎监听端口无效"), TraceLevel_Exception);
		return false;
	}
	...
	try
	{
		...
		if (m_hCompletionPort == NULL) throw TEXT("亿网棋牌网络引擎完成端口创建失败");
		...
		if (m_hServerSocket == INVALID_SOCKET) throw TEXT("亿网棋牌网络引擎监听 SOCKET 创建失败");
		int iErrorCode = bind(m_hServerSocket, (SOCKADDR*) & SocketAddr, sizeof(SocketAddr));
		if (iErrorCode == SOCKET_ERROR) throw TEXT("亿网棋牌网络引擎监听端口被占用，端口绑定失败");
		iErrorCode = listen(m_hServerSocket, 200);
		if (iErrorCode == SOCKET_ERROR) throw TEXT("亿网棋牌网络引擎监听端口被占用，端口监听失败");

		//启动发送队列
		bool bSuccess = m_SendQueueService.StartService();
		if (bSuccess == false) throw TEXT("亿网棋牌网络引擎发送队列服务启动失败");

		//建立读写线程
		for (i = 0; i < dwThreadCount; i++)
		{
			CServerSocketRSThread * pServerSocketRSThread = new CServerSocketRSThread();
			if (pServerSocketRSThread == NULL) throw TEXT("亿网棋牌网络引擎读写线程服务创建失败");
			bSuccess = pServerSocketRSThread->InitThread(m_hCompletionPort);
			if (bSuccess == false) throw TEXT("亿网棋牌网络引擎读写线程服务配置失败");
		...
		if (bSuccess == false) throw TEXT("亿网棋牌网络引擎网络监听线程服务配置");
		...
		for (i = 0; i < dwThreadCount; i++)
		{
			...
			if (bSuccess == false) throw TEXT("亿网棋牌网络引擎读写线程服务启动失败");
		}

		//网络检测线程
		m_SocketDetectThread.InitThread(this);
		bSuccess = m_SocketDetectThread.StartThread();
		if (bSuccess == false) throw TEXT("亿网棋牌网络引擎检测线程服务启动失败");

		//运行应答线程
		bSuccess = m_SocketAcceptThread.StartThread();
		if (bSuccess == false) throw TEXT("亿网棋牌网络引擎监听线程服务启动失败");
