...
bool  CAttemperEngine::StartService()
{
	...
	if (m_bService == true)
	{
		CTraceService::TraceString(TEXT("亿网棋牌调度引擎重复启动，启动操作忽略"), TraceLevel_Warning);
		return true;
	}
	...
	{
		CTraceService::TraceString(TEXT("亿网棋牌调度引擎外挂服务不存在"), TraceLevel_Exception);
		return false;
	}
	...
	{
		CTraceService::TraceString(TEXT("亿网棋牌调度引擎与队列服务绑定失败"), TraceLevel_Exception);
		return false;
	}
	...
	{
		CTraceService::TraceString(TEXT("亿网棋牌调度引擎外挂服务启动失败"), TraceLevel_Exception);
		return false;
	}
	...
	{
		CTraceService::TraceString(TEXT("亿网棋牌调度引擎队列服务启动失败"), TraceLevel_Exception);
		return false;
	}
...
bool  CAttemperEngine::SetAttemperEngineSink(IUnknownEx * pIUnknownEx)
{
	...
	if (m_pIAttemperEngineSink == NULL)
	{
		CTraceService::TraceString(TEXT("亿网棋牌调度引擎外挂服务接口获取失败，挂接操作失败"), TraceLevel_Exception);
		return false;
	}
