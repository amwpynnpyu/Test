...
bool  CTCPSocketService::StartService()
{
	...
	if (m_bService == true)
	{
		CTraceService::TraceString(TEXT("亿网棋牌网络服务重复启动，启动操作忽略"), TraceLevel_Warning);
		return true;
	}
	...
	{
		CTraceService::TraceString(TEXT("亿网棋牌网络服务与触发服务绑定失败"), TraceLevel_Exception);
		return false;
	}
...
bool  CTCPSocketService::SetTCPSocketEvent(IUnknownEx * pIUnknownEx)
{
	...
	if (m_bService == true)
	{
		CTraceService::TraceString(TEXT("亿网棋牌网络组件处于服务状态，绑定操作忽略"), TraceLevel_Warning);
		return false;
	}
	...
	{
		CTraceService::TraceString(TEXT("亿网棋牌网络组件与触发服务绑定失败"), TraceLevel_Warning);
		return false;
	}
